...
디시인사이드의 기본 레이아웃을 사용자가 원하는 방식으로 변경할 수 있는 기능을 제공합니다. 컴팩트 모드, 불필요한 요소 숨기기, 본문 영역 확장 등 다양한 옵션을 통해 사용자 경험을 개선합니다.

## 설정 옵션

| 옵션                    | 설명                                        | 기본값   |
|-----------------------|-------------------------------------------|-------|
| 컴팩트 모드 활성화 조건         | 브라우저 가로가 이 값보다 작을 경우 컴팩트 모드를 활성화합니다.      | 900px |
...
| 잡다 링크 숨기기             | 이슈줌, 타갤 개념글, 뉴스, 힛갤 등의 컨텐츠를 오른쪽 영역에서 숨깁니다. | 꺼짐    |
...
2. 원하는 설정 옵션을 켜서 디시인사이드의 레이아웃을 커스터마이징합니다.
3. 브라우저 창의 크기에 따라 컴팩트 모드가 자동으로 적용되거나, 설정에 따라 강제 적용됩니다.

## 컴팩트 모드

컴팩트 모드는 화면 공간을 효율적으로 사용하기 위해 디시인사이드의 레이아웃을 간소화합니다. 작은 화면이나 모바일 기기에서 특히 유용합니다.

- **컴팩트 모드 활성화 조건** — 브라우저 너비가 설정한 값 이하일 때 자동으로 컴팩트 모드가 활성화됩니다. (범위: 100px ~ screen.width)
- **컴팩트 모드 강제 사용** — 브라우저 너비와 관계없이 항상 컴팩트 모드를 사용합니다.
- **컴팩트 모드 강제 사용 (게시글 보기)** — 게시글 보기 페이지에서도 컴팩트 모드를 적용합니다.

## 불필요한 요소 숨기기

다양한 옵션을 통해 필요하지 않은 요소를 숨길 수 있습니다:
